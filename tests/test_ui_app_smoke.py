...
def test_ui_paste_mode_claude_code_analyze_applies(tmp_path) -> None:
    cfg = ctrl.PipelineConfig.from_base_dir(tmp_path)
    at = _fresh(tmp_path)  # default source = 내용 붙여넣기
    at.text_area[0].set_value("회사 회식 썰\n실적은 최고인데 회식은 최악인 하루였다.").run()
    # the analyze prompt + JSON box appear; paste a distilled {title, summary}
    at.text_area[1].set_value('{"title": "반전 있는 회식 썰", "summary": "실적 최고, 회식 최악"}').run()
    at = _click(at, "분석 적용")
    assert not at.exception
    at = _click(at, "프로젝트 만들기")
    pid = at.session_state["project_id"]
    assert ctrl.load_candidate(cfg, pid)["title"] == "반전 있는 회식 썰"
